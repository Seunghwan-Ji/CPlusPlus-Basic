...
void Sort(tArr* _pArr)
{
	...
	while (true)
	{
		...
		int iLoop = _pArr->iCount - 1; // 데이터 수가 n개라면, 비교하는 횟수는 n-1번이다.
		...
		for (int i = 0; i < iLoop; ++i)
		{
			...
			{
				int iTemp = _pArr->pInt[i]; // 옮길 값을 지역변수에 임시 저장.
				_pArr->pInt[i] = _pArr->pInt[i + 1]; // 교체.
				_pArr->pInt[i + 1] = iTemp; // 저장해둔 값 적용.
